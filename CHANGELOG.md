...
### Fixed
- **FleetView is visually separated from Pi's statusline.** A muted full-width divider now renders after the below-editor `main` and subagent roster, making the navigator distinct from the model, context, and repository status rows beneath it without changing keyboard navigation.
